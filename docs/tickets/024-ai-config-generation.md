...
- [ ] `figma-tokens ai` subcommand (or `figma-tokens init --ai`)
...
- [ ] AI returns: a complete `figma-tokens.config.ts`, any needed custom transforms, any needed custom formatters
...
async function generateConfig(analysis: CollectionAnalysis[], userDescription: string) {
  const systemPrompt = `You are an expert in design systems and the figma-tokens CLI tool.
Given a Figma file structure analysis and a user's requirements, generate a valid figma-tokens.config.ts file.
