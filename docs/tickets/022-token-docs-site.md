...
Add a `figma-tokens docs` command that generates a static HTML documentation site from the token files, providing a visual reference for designers and developers. The site shows all tokens, their values, alias chains, and usage examples.
...
- [ ] `figma-tokens docs` generates a static site to `build/docs/` (configurable)
- [ ] Site sections:
...
Add a workflow that runs `figma-tokens docs` and pushes to `gh-pages` branch on every merge to main that changes token files.
