 ...
 *
 * Loads and validates a dta.config.ts file at runtime.
  ...
  if (!fs.existsSync(resolved)) {
    throw new Error(
      `Config file not found: ${resolved}\n` +
        'Run "dta init" to create one, or use --config to specify a path.',
    )
  }
